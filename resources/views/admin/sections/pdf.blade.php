    ...
    <table id="emp">
        ...
        <tbody>
            @foreach($sections as $section)
                <?php 
                    $name_val = $Arabic->utf8Glyphs($section->name);
                ?>
                <tr>
                    <td>{{ $section->id }}</td>
                    <td>{{ $section->created_at }}</td>
                    <td>{{ $name_val }}</td>
                    <td>{{ $section->is_activate == 1 ? $activee_table : $not_Active_table  }}</td>
                    <td>{{ $section->archive == 1 ? $archivee_table : $not_archive_table  }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
